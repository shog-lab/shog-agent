/**
 * Mount Security Module for ShogAgent
 *
 * Validates additional mounts against an allowlist stored OUTSIDE the project root.
 * This prevents container agents from modifying security configuration.
 *
 * Allowlist location: ~/.config/shog-agent/mount-allowlist.json
 */
import fs from 'fs';
import os from 'os';
...
/**
 * Validate code repository mounts for a group.
 * Code repos are always read-write and mounted at /workspace/repos/{basename}.
 * Reuses the same allowlist validation as additionalMounts.
 */
export function validateCodeRepoMounts(
  repoPaths: string[],
  groupName: string,
  isMain: boolean,
  ...
  for (const repoPath of repoPaths) {
    ...
    if (result.allowed) {
      // Deduplicate container path basenames
      let name = path.basename(result.realHostPath!);
      ...
      validatedMounts.push({
        hostPath: result.realHostPath!,
        containerPath: `/workspace/repos/${name}`,
        readonly: result.effectiveReadonly!,
      });
