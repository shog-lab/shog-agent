 ...
 * This file should be stored at ~/.config/shog-agent/mount-allowlist.json
...
export interface Channel {
  ...
  // Optional: send an image by URL. Channels that support it implement it.
  sendImage?(jid: string, imageUrl: string, caption?: string): Promise<void>;
  // Optional: typing indicator. Channels that support it implement it.
  setTyping?(jid: string, isTyping: boolean): Promise<void>;
  // Optional: sync group/chat names from the platform.
  syncGroups?(force: boolean): Promise<void>;
  // Optional: if true, the platform already filters messages (e.g. DingTalk only
  // forwards @mentioned messages), so shog-agent skips its own trigger detection.
  handlesOwnTrigger?: boolean;
}
