...
description: Add a macOS menu bar status indicator for ShogAgent. Shows a bolt icon with a green/red dot indicating whether ShogAgent is running, with Start, Stop, and Restart controls. macOS only.
---

# Add macOS Menu Bar Status Indicator

Adds a persistent menu bar icon that shows ShogAgent's running status and lets the user
...
```bash
launchctl list | grep com.shog-agent.statusbar
```
...
Create `~/Library/LaunchAgents/com.shog-agent.statusbar.plist`, substituting the actual values
...
<dict>
    <key>Label</key>
    <string>com.shog-agent.statusbar</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.shog-agent.statusbar.plist
```
...
> The bolt icon should now appear in your macOS menu bar. Click it to see ShogAgent's status and control the service.
>
> - **Green dot** — ShogAgent is running
> - **Red dot** — ShogAgent is stopped
>
...
```bash
launchctl unload ~/Library/LaunchAgents/com.shog-agent.statusbar.plist
rm ~/Library/LaunchAgents/com.shog-agent.statusbar.plist
rm dist/statusbar
```
