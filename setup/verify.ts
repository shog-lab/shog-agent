...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.shog-agent')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.shog-agent'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active shog-agent`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
  ...
  if (
    fs.existsSync(
      path.join(homeDir, '.config', 'shog-agent', 'mount-allowlist.json'),
    )
  ) {
