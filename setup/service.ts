...
<dict>
    ...
    <string>${projectRoot}/logs/shog-agent.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/shog-agent.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.shog-agent');
  } catch {
...
/**
 * Kill any orphaned shog-agent node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned shog-agent processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/shog-agent.service';
    ...
    unitPath = path.join(unitDir, 'shog-agent.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=ShogAgent Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/shog-agent.log
StandardError=append:${projectRoot}/logs/shog-agent.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned shog-agent processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable shog-agent`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start shog-agent`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active shog-agent`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-shog-agent.sh');
  const pidFile = path.join(projectRoot, 'shog-agent.pid');

  const lines = [
    '#!/bin/bash',
    '# start-shog-agent.sh — Start ShogAgent without systemd',
    ...
    '    echo "Stopping existing ShogAgent (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting ShogAgent..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/shog-agent.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/shog-agent.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "ShogAgent started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/shog-agent.log"`,
  ];
