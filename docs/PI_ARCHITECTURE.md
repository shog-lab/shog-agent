...
- [ ] **meta-agent 管理 ShogAgent 代码**：shog 作为 meta-agent，通过 L2 repo sub-agent 改 ShogAgent 自身代码（当前已支持 codeRepos 挂载，需补充稳定性与治理约束）

#### 4. 其他
- [ ] **LongMemEval 词项匹配版消融实验**：差最后一组基线数据
- [ ] **SWE-Bench**：验证 L2 repo sub-agent 的代码修复能力
- [ ] **钉钉端到端验证**：黑盒测试通过 cli 验证通过，但钉钉群触发未验证

### 已完成

- ~~OneCLI 集成~~ → 已删除，用 native credential proxy
- ~~每个 group 独立进化~~ → 已集中到 meta-agent（meta-triage + daily-audit）
...
- ~~黑盒测试~~ → L2 repo sub-agent + agent-browser + artifacts 目录
...
### raw/conversations/ 和 memory/ 的职责分离

| 目录 | 写入者 | 内容 | 用途 |
|------|--------|------|------|
| `raw/conversations/` | archiveTranscript（自动） | 对话全文 dump | 审计日志，给你看的 |
...
当前只有 meta-agent group 跑 evolution-loop（凌晨 3 点），普通 group 不独立进化。原因：

- 不是架构限制——每个 group 都有自己的 conversations 和 memory，技术上可以独立复盘
- 是实际需求不足——当前普通 group 数量少、对话量不大，单独跑一个容器做进化投入产出不合理
- meta-agent group 有全局视角（只读挂载了所有 group 的 conversations），可以统一复盘，发现问题后通过治理流程审核和修正
...
| 记忆系统 | raw/conversations（全量 dump） | mem extension（选择性） + archiveTranscript（审计） |
...
- **L1 负责理解需求、拆任务、生成 PRD / 计划内容，但不直接写 repo。**
- **如果需要把 PRD、progress 等文件落到 repo，由 L2 先写入，再继续执行。**
...
| `groups/{name}/raw/conversations/` | 对话日志（archiveTranscript） |
...
- **raw/conversations/ 是唯一数据源** — archiveTranscript 的全量日志在此体现价值
