/**
 * Web tools for pi-coding-agent
 * Provides web_search and web_fetch capabilities.
 */
...
const WebFetchParams = Type.Object({
  ...
  max_length: Type.Optional(
    Type.Number({
      description:
        'Maximum response length in characters (default: 20000)',
    }),
  ),
});
...
async function duckDuckGoSearch(
  ...
  const res = await fetch(url, {
    headers: {
      'User-Agent':
        'Mozilla/5.0 (compatible; ShogAgent/1.0; +https://github.com/shog-lab/shog-agent)',
    },
  });
...
const webSearchTool: ToolDefinition<typeof WebSearchParams> = {
  name: 'web_search',
  label: 'Web Search',
  description:
...
const webFetchTool: ToolDefinition<typeof WebFetchParams> = {
  name: 'web_fetch',
  label: 'Web Fetch',
  description:
...
export function createWebTools(): AnyToolDef[] {
